...
function Room() {
  ...
  return (
    <>
      <h1>채널 접속</h1>
      <label>채널 명</label>
      <input type="text" placeholder="채널 명" onChange={handelChannelName} />
      <label>채널 Token</label>
      <input type="text" placeholder="채널 토큰" onChange={handelToken} />
      <br />
      <button onClick={rtcJoin}>Channel Join</button>
      <button onClick={rtcLeave}>Channel Leave</button>
      <p>채널 : 상태 {text}</p>
      <br />
      <label>Local Volumn</label>
      <input
        type="range"
        min={0}
        max={100}
        step={1}
        onChange={localAudioChange}
      />
      <label>Remote Volumn</label>
      <input
        type="range"
        min={0}
        max={100}
        step={1}
        onChange={remoteAudioChange}
      />
    </>
  );
}
